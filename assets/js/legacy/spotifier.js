...
async function spotifyGetInfo() {
    ...
    try {
        console.log(`${getApiUrl()}${API_ENDPOINTS.PLAYLIST_INFO}`);
        const response = await fetch(`${getApiUrl()}${API_ENDPOINTS.PLAYLIST_INFO}`, {
            method: 'POST',
...
async function spotifyDownload() {
    ...
    try {
        const response = await fetch(`${getApiUrl()}${API_ENDPOINTS.PLAYLIST_DOWNLOAD}`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                url: url,
        ...
        if (data.success) {
            ...
            spotifyResult.innerHTML = `
                <div class="success-message">
                    ...
                    ${data.download_url ? `
                        <a href="${getApiUrl()}${data.download_url}" download="${data.filename}" class="download-btn">
...
async function spotifyListFiles() {
    ...
    try {
        const response = await fetch(`${getApiUrl()}${API_ENDPOINTS.FILES_LIST}`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                spotifyKey: spotifyKey.value
            ...
            const filesHtml = data.files.map(file => `
                <div class="file-item">
                    ...
                    <div class="file-actions">
                        <a href="${getApiUrl()}${API_ENDPOINTS.DOWNLOAD_FILE}${data.key}${file.name}" class="btn-download-small" download>
...
async function spotifyDeleteFile(key,filename) {
    ...
    try {
        const response = await fetch(`${getApiUrl()}${API_ENDPOINTS.DELETE_FILE}${key}${filename}`, {
            method: 'DELETE'
        });
...
async function spotifyCleanup() {
    ...
    try {
        key = `/${key}`;
        const response = await fetch(`${getApiUrl()}${API_ENDPOINTS.CLEANUP}${key}`, {
            method: 'DELETE'
        });
